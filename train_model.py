...
joblib.dump(imputer, 'imputer.pkl')  # Save imputer
...
joblib.dump(selector, 'selector.pkl')  # Save selector
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
    ...
    catboost_preds += model.predict_proba(X_test)[:, 1] / kf.n_splits
    if fold == 0:  # Save only one model for deployment
